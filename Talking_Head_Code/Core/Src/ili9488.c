...
void ILI9488_Init()
{
	...
	RST_A();
	 ...
	 ILI9488_SendData(0x0F);
	 	...
	 	ILI9488_SendData(0x66); 	  //0x66: 18 bit 0x61: 3bit
...
void drawPixel(int16_t x, int16_t y, uint16_t color)
{
	...
	write16BitColor(color);
    //write3BitColor(color);  //0x03: cyan
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
...
{
    uint8_t r = (color3bit & 0x04) ? 255 : 0;  // Red bit (bit 2)
    uint8_t g = (color3bit & 0x02) ? 255 : 0;  // Green bit (bit 1)
    uint8_t b = (color3bit & 0x01) ? 255 : 0;  // Blue bit (bit 0)
